...
namespace Sidekick_E_Invoicing
{
    public class DashboardForm : Form
    {
        ...
        // Brand colours
        private static readonly Color NavyDark   = ColorTranslator.FromHtml("#1d4354");
        private static readonly Color NavyMid    = ColorTranslator.FromHtml("#1b6656");
        private static readonly Color NavyLight  = ColorTranslator.FromHtml("#1b6656");
        private static readonly Color AccentGold = ColorTranslator.FromHtml("#7bb06b");
            ...
            out Button btnLogout)
        {
            ...
            {
                Image     = LoadImageSafely("Sidekick-LOGO-SIDEBAR.png"),
        ...
        private Panel BuildHeaderBanner()
        {
            ...
            {
                Text      = "Sidekick E-Invoicing System Dashboard",
            ...
            Button btnPayments, Button btnReports, Button btnLogout)
        {
            ...
            btnCustomers.Click    += (s, e) => Go(new CustomerForm(),           btnCustomers,    "Sidekick e-Invoice — Customers");
            btnProducts.Click     += (s, e) => Go(new ProductForm(),            btnProducts,     "Sidekick e-Invoice — Products");
            btnInvoice.Click      += (s, e) => Go(new GenerateInvoiceForm(),    btnInvoice,      "Sidekick e-Invoice — New Invoice");
            btnViewInvoices.Click += (s, e) => Go(new InvoiceViewerForm(),      btnViewInvoices, "Sidekick e-Invoice — Invoice List");
            btnSeller.Click       += (s, e) => Go(new SellerForm(),             btnSeller,       "Sidekick e-Invoice — Sellers");
            btnPayments.Click     += (s, e) => Go(new PaymentForm(),            btnPayments,     "Sidekick e-Invoice — Payments");
            btnReports.Click      += (s, e) => Go(new ReportingForm(),          btnReports,      "Sidekick e-Invoice — Reports");
            btnLogout.Click += (s, e) =>
            {
                _loggingOut = true;
    ...
    public class InvoiceStatusChart : Panel
    {
        ...
        private void DrawLegendItem(Graphics g, int x, int y, Color c, string text)
        {
            ...
            using (var f = new Font("Segoe UI", 8.5f, FontStyle.Bold))
            using (var br = new SolidBrush(ColorTranslator.FromHtml("#1b6656")))
